...
class ACL {
    ...
    check(req, res, next) {
        ...
        if (req && req.user && req.user.roles) {
            ...
            if (_.includes(roles, acl.superRole)) { 
                access = true; 
            }
    ...
    roleAccess(resource, method) {
        ...
        _.each(acl.rules, (rule) => {
            ...
            if (!route) {
                ...
                if (_.isString(route.methods)) { 
                    route.methods = _.toLower(route.methods); 
                }
                else { 
                    route.methods = _.map(route.methods, _.toLower); 
                }
    ...
    makeRoles(userRoles) {
        ...
        _.each(userRoles, (role) => {
            ...
            if (_.isString(roleRules.includeRoles)) { 
                includeRoles.push(_.toLower(roleRules.includeRoles)); 
            }
            else { 
                includeRoles = _.map(roleRules.includeRoles, _.toLower); 
            }

            _.each(includeRoles, (role) => { 
                roles.push(role); 
            });
        })
